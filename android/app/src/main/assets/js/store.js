...
export const Store = {
  ...
  async seedExerciseTypes() {
    ...
    const defaults = [
      ...
      { name: '卷腹', category: 'core', emoji: '🧘', isCustom: false },
      { name: '俯卧撑', category: 'strength', emoji: '💪', isCustom: false },
      { name: '死虫式', category: 'core', emoji: '🧘', isCustom: false },
  ...
  // 一次性迁移：把错误归到「力量」的内置项目(死虫式/卷腹)与同名历史打卡纠正为「核心」
  async migrateExerciseData() {
    const canonical = {
      '死虫式': { category: 'core', emoji: '🧘' },
      '卷腹':   { category: 'core', emoji: '🧘' },
    };
    // 1) 纠正内置运动类型（仅非自定义）
    const types = await DB.getAll('exerciseTypes');
    for (const t of types) {
      if (t.isCustom) continue;
      const fix = canonical[t.name];
      if (fix && (t.category !== fix.category || t.emoji !== fix.emoji)) {
        await DB.put('exerciseTypes', { ...t, category: fix.category, emoji: fix.emoji });
      }
    }
    // 2) 纠正历史打卡记录（按名称，仅当类别为 strength 时）
    const workouts = await DB.getAll('workouts');
    for (const w of workouts) {
      const fix = canonical[w.exerciseName];
      if (fix && w.category === 'strength') {
        await DB.put('workouts', { ...w, category: fix.category, emoji: fix.emoji });
      }
    }
  },
  ...
  async updateExerciseType(data) {
    await DB.put('exerciseTypes', data);
    emit('exercise-type-change');
  },
  async updateWorkoutsByExerciseName(oldName, patch) {
    const all = await DB.getAll('workouts');
    for (const w of all) {
      if (w.exerciseName === oldName) {
        await DB.put('workouts', { ...w, ...patch });
      }
    }
  },
  async restoreDefaultTypes() {
    ...
    for (const d of defaults) {
      if (!existing.some(t => !t.isCustom && t.name === d.name)) {
        await DB.add('exerciseTypes', d);
      }
    }
  },
  ...
  async importAllData(file) {
    ...
    for (const name of stores) {
      ...
      await DB.clear(name);
      await DB.bulkAdd(name, data[name]);
    }
    emit('todo-change');
    emit('vlog-change');
    emit('workout-change');
