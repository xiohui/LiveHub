    ...
    <div class="page" id="page-learn">
      <div class="page-inner">
        ...
        <div class="page-scroll">
          ...
          <div class="segmented">
            <button class="seg-item ${currentTab === 'reading' ? 'active' : ''}" data-tab="reading">${icon('book')} 读书</button>
            <button class="seg-item ${currentTab === 'materials' ? 'active' : ''}" data-tab="materials">${icon('tag')} 资料</button>
          </div>
...
async function renderReading() {
  ...
  if (readings.length === 0) {
    container.innerHTML = `
      <div class="empty-state"><div class="icon">${icon('book')}</div><div class="text">还没有书籍，添加一本开始阅读吧</div></div>
    ...
    if (finished.length > 0) {
      html += `<div class="section-title" style="margin-top:12px;"><h3>${icon('check')} 已完成</h3></div><div class="book-scroll">`;
...
async function renderMaterialList(category) {
  ...
  if (filtered.length === 0) {
    list.innerHTML = `<div class="empty-state"><div class="icon">${icon('book')}</div><div class="text">该分类暂无资料</div></div>`;
    return;
  }
  ...
  list.innerHTML = filtered.map(m => `
    <div class="material-item glass-sm ${m.isManual ? 'mat-pinned' : ''}" data-id="${m.id}">
      ...
      ${m.url ? `<a href="${escHtml(m.url)}" target="_blank" class="arrow" style="text-decoration:none;">${icon('chevron')}</a>` : '<span class="arrow" style="cursor:pointer;">📖</span>'}
