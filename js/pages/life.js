  ...
  return `
    ...
    <div class="form-group">
      ...
      <div class="option-group" id="f-todo-pri">
        <button class="option-btn" data-v="high">🔴 高</button>
        <button class="option-btn selected" data-v="medium">🟡 中</button>
        <button class="option-btn" data-v="low">🟢 低</button>
      </div>
    </div>
    <div class="form-group">
...
async function renderTodoList() {
  ...
  if (todos.length === 0) {
    container.innerHTML = '<div class="empty-state"><div class="icon">📋</div><div class="text">还没有待办，添加一条吧</div></div>';
      ...
      <div class="todo-list">
        ${todos.sort((a,b) => {
          ...
          <div class="todo-item glass-sm" data-id="${t.id}">
            ...
            <button class="todo-delete" data-action="delete-todo">✕</button>
          </div>
        `).join('')}
      </div>
...
async function renderVlogList() {
  ...
  if (vlogs.length === 0) {
    container.innerHTML = '<div class="empty-state"><div class="icon">📝</div><div class="text">还没有 Vlog 记录</div></div>';
    const addBtn = document.createElement('button');
    addBtn.className = 'add-btn';
      ...
      ${vlogs.map(v => `
        <div class="vlog-item glass">
          <div class="v-date">
            ...
            <span>${v.date || '---'}</span>
            <button class="vlog-delete" data-id="${v.id}" style="margin-left:auto;background:none;border:none;color:var(--text-muted);font-size:12px;cursor:pointer;opacity:0.3;">✕</button>
          </div>
    ...
    <div class="page" id="page-life">
      <div class="page-inner">
        ...
        <div class="page-scroll">
          ...
          <div class="segmented">
            <button class="seg-item ${currentTab === 'todo' ? 'active' : ''}" data-tab="todo">✅ 待办</button>
            <button class="seg-item ${currentTab === 'vlog' ? 'active' : ''}" data-tab="vlog">📝 Vlog</button>
          </div>
