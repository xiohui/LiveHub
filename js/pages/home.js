      ...
      <div class="page-inner">
        ...
        <div class="page-scroll">
          ...
          <div class="quick-actions">
            <button class="qa-item" data-action="todo"><div class="qa-icon">📋</div><span>新建待办</span></button>
            <button class="qa-item" data-action="sport"><div class="qa-icon">🏃</div><span>运动打卡</span></button>
            <button class="qa-item" data-action="reading"><div class="qa-icon">📖</div><span>记录阅读</span></button>
            <button class="qa-item" data-action="vlog"><div class="qa-icon">📝</div><span>写 Vlog</span></button>
          </div>

          <div class="section-title"><h3>📡 最近动态</h3></div>
...
function showSettingsModal() {
  ...
  overlay.innerHTML = `
    <div class="modal-sheet">
      ...
      <h3 style="font-size:20px;font-weight:700;margin-bottom:20px;">⚙️ 设置</h3>
      <div class="form-group">
        ...
        <div class="btn-group" style="flex-direction:column;gap:8px;">
          <button class="btn btn-gradient btn-block" id="btn-export">📤 导出数据</button>
          <button class="btn btn-glass btn-block" id="btn-import">📥 导入数据</button>
        </div>
      </div>
  ...
  document.getElementById('import-file-input').addEventListener('change', async e => {
    ...
    try {
      ...
      const status = overlay.querySelector('#import-status');
      status.textContent = '✅ 数据已恢复，刷新页面查看';
      status.style.display = 'block';
      status.style.color = 'var(--success)';
    } catch (err) {
      const status = overlay.querySelector('#import-status');
      status.textContent = '❌ 导入失败：文件格式不正确';
      status.style.display = 'block';
